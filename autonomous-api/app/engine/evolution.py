...
class EvolutionEngine:
    ...
    async def run_async(self, generations: int = 10, population_size: int = 10, use_docker: bool = False) -> dict:
        ...
        try:
            await self._emit_update({"type":"evolution_start","run_id":run_id,"generations":generations,"population_size":population_size,"evaluation_mode":"runtime" if use_docker else "static"}, run_id=run_id)
            population = Population(size=population_size); history = []; best_genome = None; best_fitness = float("-inf"); output_path = None
            for gen in range(generations):
                ...
                for genome in population.individuals:
                    ...
                    if use_docker and evidence["evaluation_mode"] != "runtime":
                        fitness = 0.0
            ...
            result = {"run_id":run_id,"best_genome":best_genome.encode() if best_genome else None,"best_fitness":best_fitness if best_genome else 0.0,"production_readiness":self.production_analyzer.analyze(best_genome) if best_genome else None,"history":history,"output_path":output_path,"total_generations":generations}
            db = SessionLocal()
            try:
