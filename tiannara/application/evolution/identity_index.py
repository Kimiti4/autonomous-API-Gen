"""R2.10.4 — SemanticIdentityIndex: the single identity namespace.
...
The index owns:
  ...
  * ``genes`` / ``gene_hashes`` / ``gene_hash`` — (domain, gene_id) ->
    gene object / canonical hash, using the same canonicalization as the
    ISR's content hash (``canonicalize``), so per-gene hashes compose with
    the whole-ISR identity.
  * ``replace_gene``      — deterministic single-gene replacement producing
    a new ISR version (``with_system`` provenance), raising KeyError for
    unknown genes (pre-execution rejection).
...
This is the load-bearing mechanism of the R2.10.4 SemanticEvolutionGate:
locality, reference integrity, and the J protection diff all share the ONE
namespace, so a change can never be judged in a different namespace than it
was authored in.
"""
...
class SemanticIdentityIndex:
    """The single namespace for semantic identity resolution (R2.10.4)."""

    # -- the J walk: projection path -> identity id ---------------------------

    def path_identities(self, isr: Any) -> dict[str, str]:
    ...
    def identity_for_path(self, isr: Any, path: str) -> str:
        return self.path_identities(isr).get(path) or path

    # -- (domain, gene_id) addressing ------------------------------------------

    def genes(self, isr: Any) -> dict[tuple[str, str], Any]:
    ...
    def gene_hashes(self, isr: Any) -> dict[tuple[str, str], str]:
        """(domain, gene_id) -> canonical gene hash for the whole ISR."""
        return {
            key: _gene_hash(gene)
            for key, gene in self.genes(isr).items()
        }

    def gene_hash(self, isr: Any, domain: str, gene_id: str) -> str:
        try:
            return _gene_hash(self.genes(isr)[(domain, gene_id)])
        except KeyError as exc:
            raise KeyError(
                f"no '{domain}' gene '{gene_id}' in the identity index"
            ) from exc

    # -- deterministic single-gene replacement ----------------------------------

    def replace_gene(self, isr: Any, domain: str, gene_id: str, new_gene: Any):
        """Return a new ISR version with exactly one gene replaced.

        Replacement is mechanical (no validation sweep): the R2.10.4 gate
        evaluates the candidate and its proofs; structural validation stays
        a separate pre-execution concern. Unknown genes raise KeyError —
        a delta addressing a gene that does not exist is rejected before
        evaluation, never silently ignored.
        """
        system = isr.system
        if domain in ("behavior", "migration", "temporal"):
            for mi, module in enumerate(system.modules):
                new_module = self._replace_in_module(
    ...
    def _replace_in_module(
        self, module: Any, domain: str, gene_id: str, new_gene: Any
    ) -> Any:
        if domain == "behavior":
            for wi, workflow in enumerate(module.workflows):
    ...
    # -- cross-gene reference integrity -----------------------------------------

    def resolvable_ids(self, isr: Any) -> frozenset[str]:
        ...
        system = isr.system
        ids = {gene_id for (_, gene_id) in self.genes(isr)}
