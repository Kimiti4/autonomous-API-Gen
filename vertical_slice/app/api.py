"""VS-D04 — product interface layer (api-identity, api-task, api-workspace).

FastAPI routes only; no server is started here (no deployment in VS-D04).
Every route maps to an ISR API node; auth is Bearer sessions, errors explicit.
"""
...
def create_app(store: TaskTrackerStore,
    ...
    service = TaskTrackerService(store, token_generator=token_generator)
    app = FastAPI(title="vs1-task-tracker")
