"""VS-D15 — product interface layer, central-policy profile.

Same route contract as vertical_slice.app.api (api-identity, api-task,
api-workspace); wired to TaskTrackerServiceV2. No server started here.
"""
...
from vertical_slice.app_v2.service import (
    _UNSET,
    AuthError,
    AuthorizationError,
...
def create_app(store: TaskTrackerStore,
    ...
    service = TaskTrackerServiceV2(store, token_generator=token_generator)
    app = FastAPI(title="vs1-task-tracker-v2")
    ...
    def create_task(workspace_id: str, body: TaskCreateBody,
        ...
        try:
            task = service.create_task(token, workspace_id, body.title, body.status)
        except (ValidationError, AuthError, AuthorizationError) as exc:
            raise _http_error(exc)
        return _task_json(task)

    @app.get("/workspaces/{workspace_id}/tasks")
    def list_tasks(workspace_id: str, token: str = Depends(_token)) -> list[dict[str, object]]:
        try:
            tasks = service.list_tasks(token, workspace_id)
        except (ValidationError, AuthError, AuthorizationError) as exc:
            raise _http_error(exc)
    ...
    def update_task(workspace_id: str, task_id: str, body: TaskUpdateBody,
        ...
        try:
            ...
            task = service.update_task(token, task_id, title=body.title,
                                       status=body.status, assignee_id=assignee)
        except (ValidationError, AuthError, AuthorizationError) as exc:
            raise _http_error(exc)
        if task.workspace_id != workspace_id:
            raise HTTPException(status_code=403, detail="caller is not a workspace member")
